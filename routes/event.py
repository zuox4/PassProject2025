...
def get_events(address: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    data = db.query(Event).filter(Event.datetime_start.between(start, end) & (Event.address == address)).all()

    events = [{"id": i.id, "title": i.title, "placeevent": i.placeevent, "address": i.address,
                'eventers': i.eventers, "datetime_start": i.datetime_start, "datetime_end": i.datetime_end, "creater_name": i.creater_name, 'creater_email': i.creater_email} for i in data]
    return events
...
@router.post('/event/confirm_event/{eventer_id}')
...
@router.post('/event/confirm_event_out/{eventer_id}')
def out_eventer(eventer_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    eventer = db.query(Eventer).filter(Eventer.id == eventer_id).first()
    eventer.status = False
    db.commit()
    db.refresh(eventer)
    return {
        "id": eventer.id,
